#원격DB(MariaDB)와 연동하여 자료를 읽은 후 jikwon 테이블로 근무년수에 따른
#연봉 예측 모델 작성
...
conn <- dbConnect(drv, "jdbc:mysql://127.0.0.1:3306/test","root","")
dbListTables(conn)

#query <- "select jikwon_no, jikwon_name, buser_num, jikwon_jik, jikwon_pay, 
#date_format(now(),'%Y')-date_format(jikwon_ibsail,'%Y') + 1 as jikwon_ibsa, 
#jikwon_gen, jikwon_rating from jikwon"
...
summary(model) # p-value: 6.943e-13 < 0.05 유의한 모델 R-squared: 0.8458 84.58% 설명력을가짐
